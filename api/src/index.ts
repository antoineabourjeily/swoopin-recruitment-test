...
async function launchServer(conf: any, logger: ILogger) {
  ...
  server.register(FastifyJWT, {
    ...
    sign: {
      algorithm: "HS512",
      expiresIn: conf.authentication?.expiresIn
        ? conf.authentication.expiresIn
        : "1 hour",
    },
  });
  ...
  server.decorate(
    "authenticateAccount",
    (req: any, res: any, done: () => void) =>
      AuthenticationService.authenticateAccount({ req, res, done })
  );
...
async function run() {
  ...
  const conf = await loadConfiguration(
    path.resolve("../conf/application.yml"),
    logger
  );
