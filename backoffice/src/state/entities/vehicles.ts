...
export default class StateVehicles {
  ...
  @action.bound async setOnline(id: string, onlineState: boolean) {
    // FIXME
    const response = await this.api?.put(
      `/vehicles/${onlineState ? "online" : "offline"}/${id}`,
      undefined,
      {
        headers: {
  ...
  @action.bound updateList(vehicles: any[]) {
    this.list = vehicles
      .slice()
      .sort((v1: Vehicle, v2: Vehicle) =>
        v1.name.toLowerCase().localeCompare(v2.name.toLowerCase())
      )
      .map((vehicle: any) => {
        return {
          ...vehicle,
          vehicle: vehicleTypes[vehicle.vehicle]
            ? vehicleTypes[vehicle.vehicle]
            : "Inconnu",
        };
      });
  }
  ...
  @computed get selectedVehicle(): Vehicle | null {
    if (this.selectedVehicleId) {
      const vehicle = this.list.find(
        (vehicle: any) => vehicle.id === this.selectedVehicleId
      );
      if (vehicle) {
        return vehicle;
      }
