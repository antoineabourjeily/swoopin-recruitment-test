...
const PageVehicles = observer(({ location }: { location: any }) => {
  ...
  const connectedState: { tab: "all" | "online" } = useLocationState(
    location,
    "connected"
  );

  const { selected, isSelected, select } = useRadioSelector(
    connectedState.tab || "all"
  );
      ...
      <div className={cx("__content")}>
        {
          ...
          state.vehicles[selected].length > 0 ? (
            // @ts-ignore
            state.vehicles[selected].map((vehicle: any) => (
              <Vehicle key={vehicle.id} {...vehicle} />
            ))
          ) : (
            <div className={cx("__empty")}>Aucun véhicule connecté</div>
          )
        }
      </div>
    </section>
