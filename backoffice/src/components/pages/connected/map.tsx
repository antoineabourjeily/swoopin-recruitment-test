...
import "./map.scss";
import { SidebarVehicle } from "components/shared/connected";
...
const PageDrivers = observer(({ location }: { location: any }) => {
  ...
  const onSideBarVehicleClick = useCallback(
    (e: any, { location: [lat, long] }: any) => {
      e.preventDefault();
      mapRef.map?.flyTo({
        center: [long, lat],
        essential: true,
    ...
    <section id="page-map" className={block}>
      ...
      <div className={cx("__content")}>
        ...
        >
          ...
          >
            {/* FIXME */}
            {state.vehicles.online.map(({ id, location: [lat, long] }) => (
              <Feature
                key={id}
                ...
                onClick={(e) => onMapVehicleClick(e, id)}
              />
            ))}
          </Layer>
          ...
          >
            {/* FIXME */}
            {state.vehicles.offline.map(({ id, location: [lat, long] }) => (
              <Feature
                key={id}
                coordinates={[long, lat]}
                onMouseEnter={() =>
        ...
        <div className={cx("__sidebar")}>
          ...
          <div className={cx("__list")}>
            {/* FIXME */}
            {state.vehicles.all.map((vehicle: any) => (
              <SidebarVehicle
                onClick={(e: any) => onSideBarVehicleClick(e, vehicle)}
                key={vehicle.id}
                {...vehicle}
              />
            ))}
